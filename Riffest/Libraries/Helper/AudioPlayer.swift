...
@objc protocol MusicPlayerDelegate : NSObjectProtocol {
    ...
    @objc optional func musicPlayer(player: MusicPlayer, didChangeTrack: RMSong)
    ...
    @objc optional func musicPlayer(player: MusicPlayer, didFinishPlayingTrack track: RMSong)
...
class MusicPlayer: NSObject, STKAudioPlayerDelegate {
    ...
    func changeTrack(atIndex index: Int, completion: ((_ error: NSError) -> Void)?) {
        ...
        debouncer.call()
        debouncer.callback = {
            let body = ["song_id": "\(self.queue![self.indexPlaying].id ?? 0)", "duration": "\(self.currentDuration!)"]
            TrackerService.onHandleTracker("mobile/song/counter","POST",body,onSuccess: { (success) in
    ...
    func next(completion: (_ error: NSError) -> Void) -> Void {
        ...
        let indexSongPath = self.indexPlaying - 1
        if indexSongPath > 0 {
            let debouncer = Debouncer(interval: 0)
            debouncer.call()
            debouncer.callback = {
                let body = ["song_id": "\(self.queue![indexSongPath].id ?? 0)", "duration": "\(self.currentDuration!)"]
                print(body)
                TrackerService.onHandleTracker("mobile/song/counter","POST",body,onSuccess: { (success) in
                    print("success")
                })
    ...
    func audioPlayer(_ audioPlayer: STKAudioPlayer, stateChanged state: STKAudioPlayerState, previousState: STKAudioPlayerState) {
        ...
        if state == STKAudioPlayerState.buffering {
            ...
            DispatchQueue.main.async {
                PlayerController.sharedController.playerButton.setImage(UIImage.init(named: "pause"), for: .normal)
                ...
                self.customTabBarController?.indicatorView.stopAnimating()
                self.customTabBarController?.indicatorView.isHidden = true
                self.customTabBarController?.btnPlayOrPause.setImage(UIImage.init(named: "pause"), for: .normal)
            ...
            DispatchQueue.main.async {
                PlayerController.sharedController.playerButton.setImage(UIImage.init(named: "play"), for: .normal)
                self.customTabBarController?.btnPlayOrPause.setImage(UIImage.init(named: "play"), for: .normal)
            }
        }
    ...
    private func updateNowPlayingInfoCenter() {
        ...
        MPNowPlayingInfoCenter.default().nowPlayingInfo = [
            ...
            MPMediaItemPropertyArtist: playlist[indexPlaying].artist_name ?? "",
