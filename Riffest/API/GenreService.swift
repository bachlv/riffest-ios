...
class GenreService {
    ...
    class func onHandleGetAllGenre(_ url: String, _ next_page: Int, _ search:String, _ onSuccess: @escaping (
        _ genre: [RMGenre]
        ) -> (), onError: @escaping (_ errorMessage: String) -> ()) {
        let urlString = baseUrl+url+"?page=\(next_page)&search=\(search)"
        GenreService.baseRequest(withURL: urlString, onSuccess: { (data) in
            var genre: [RMGenre] = []
            do {
    ...
    class func onHandleGetSongById(_ url: String ,_ id: Int,_ next_page: Int, _ search:String, _ onSuccess: @escaping (_ songs: [RMSong]) -> (), onError: @escaping (_ errorMessage: String) -> ()) {
        let urlString = baseUrl+url+"/\(id)?page=\(next_page)&search=\(search)"
        GenreService.baseRequest(withURL: urlString, onSuccess: { (data) in
            var song: [RMSong] = []
            do {
                if let json = try JSONSerialization.jsonObject(with: data!, options: JSONSerialization.ReadingOptions.allowFragments) as? NSDictionary {
