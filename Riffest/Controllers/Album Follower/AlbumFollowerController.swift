...
class AlbumFollowerController: UITableViewController {
    
    static var sharedController = AlbumFollowerController()
    ...
    var url: String = "mobile/albums/view"
    var subView = UIView()
    var songs: [RMSong] = []
    ...
    private func setupView(){
        ...
        tableView?.register(AlbumTrackCell.self, forCellReuseIdentifier: cellId)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        AlbumFollowerController.sharedController = self
        setupView()
        onHandleRequestData()
        onHandleGetSong()
    }
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = albums[currentIndex].title
    }
    ...
    @objc func onHandleFollowAndUnFollow(sender: UIButton){
        if sender.tag == 1{
            // unfollow
            let body = ["album_id": "\(albums[currentIndex].id ?? 0)"]
            RMAlbumService.onHandleFollowAndUnfollow("mobile/albums/unfollow","POST",body,onSuccess: { (follow) in
            ...
            // following
            let body = ["album_id": "\(albums[currentIndex].id ?? 0)"]
            RMAlbumService.onHandleFollowAndUnfollow("mobile/albums/following","POST",body,onSuccess: { (follow) in
                DispatchQueue.main.async {
                    sender.tag = 1
                    self.onHandleRequestData()
                }
    ...
    func onHandleRequestData(){
        let albumId = "\(albums[currentIndex].id ?? 0)"
        RMAlbumService.onHandleViewAlbumProfile(
            url+"/"+albumId,{ (album, follow) in
                DispatchQueue.main.async {
                    self.TotalFollowLabel.text = "\(album?.followers ?? 0) followers"
                    if follow == true {
    ...
    func onHandleGetSong(){
        let artistId = "\(albums[currentIndex].id ?? 0)"
        RMArtistService.onHandleGetSongByID("mobile/albums/song"+"/"+artistId,{ (song) in
            DispatchQueue.main.async {
                self.songs = song
...
extension AlbumFollowerController {
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            ...
            DispatchQueue.main.async {
                self.NameLabel.text = self.albums[self.currentIndex].title
                self.ImageView.sd_setImage(with: URL(string: (self.albums[self.currentIndex].poster!))!, placeholderImage: UIImage(named: "thumbnail"))
                self.ImageViewCover.sd_setImage(with: URL(string: (self.albums[self.currentIndex].poster!))!, placeholderImage: UIImage(named: "thumbnail"))
            }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as! AlbumTrackCell
