...
class ArtistSingleController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = artists[currentIndex].name
    }
    ...
    @objc func onHandleFollowAndUnFollow(sender: UIButton){
        if sender.tag == 1{
            // unfollow
            let body = ["artist_id": "\(artists[currentIndex].id ?? 0)"]
    ...
    func onHandleRequestData(){
        let artistId = "\(artists[currentIndex].id ?? 0)"
...
extension ArtistSingleController {
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            ...
            DispatchQueue.main.async {
                self.NameLabel.text = self.artists[self.currentIndex].name
