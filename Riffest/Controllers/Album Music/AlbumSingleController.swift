...
class AlbumSingleController: UITableViewController {
    
    static var sharedController = AlbumSingleController()
    fileprivate let cellId = "cellId"
    fileprivate let cellTrackId = "cellTrackId"
    
    var albumSong: RMSong?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        AlbumSingleController.sharedController = self
        setupView()
        onHandleRequestData()
        onHandleGetSong()
    }
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = albumSong?.album_title
    ...
    @objc func onHandleFollowAndUnFollow(sender: UIButton){
        if sender.tag == 1{
            // unfollow
            let body = ["album_id": "\(albumSong?.album_id ?? 0)"]
    ...
    func onHandleRequestData(){
        let albumId = "\(albumSong?.album_id ?? 0)"
    ...
    func onHandleGetSong(){
        let albumId = "\(albumSong?.album_id ?? 0)"
        RMAlbumService.onHandleGetSongByID("mobile/albums/song"+"/"+albumId,{ (song) in
            DispatchQueue.main.async {
                self.songs = song
...
extension AlbumSingleController {
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            ...
            DispatchQueue.main.async {
                self.NameLabel.text = self.albumSong?.album_title
                self.ImageView.sd_setImage(with: URL(string: (self.albumSong?.album_poster!)!)!, placeholderImage: UIImage(named: "thumbnail"))
                self.ImageViewCover.sd_setImage(with: URL(string: (self.albumSong?.album_poster!)!)!, placeholderImage: UIImage(named: "thumbnail"))
            }
