...
class ReleaseController: UITableViewController {
    ...
    public static var sharedController = ReleaseController()
    var url: String = "mobile/release-song"
    var search = ""
    var next_page = 1
    var last_page = 0
    
    private func setupView(){
        tableView?.register(ReleaseTrackCell.self, forCellReuseIdentifier: cellId)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        ReleaseController.sharedController = self
        setupView()
...
extension ReleaseController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as! ReleaseTrackCell
