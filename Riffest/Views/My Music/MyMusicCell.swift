...
class MyMusicAlbumCell: UITableViewCell{
    
    var album: RMAlbum? {
        didSet {
            if let _album = album {
                self.textLabel?.text = _album.title
                //self.detailTextLabel?.text = "\u{f004} \(_album.followers!) Followers"
...
//        self.detailTextLabel?.font = AppStateHelper.shared.defaultFontAwesome(size:13)
//        self.detailTextLabel?.textColor = UIColor.themeBaseColor()
//
       // self.textLabel?.frame = CGRect(x: 120, y: textLabel!.frame.origin.y, width: 230, height: textLabel!.frame.height)
      //  self.detailTextLabel?.frame = CGRect(x: 120, y: self.detailTextLabel!.frame.origin.y, width:140, height: self.detailTextLabel!.frame.height)
...
//
    override func layoutSubviews(){
        ...
        self.detailTextLabel?.frame = CGRect(x: 120, y: self.detailTextLabel!.frame.origin.y - 15, width:220, height: self.detailTextLabel!.frame.height)
        self.imageView?.frame = CGRect(x: 15.0,y: 5.0,width: 90.0,height: 90.0)
        self.imageView?.layer.cornerRadius = 5.0
        self.imageView?.clipsToBounds = true
    }
...
class MyMusicArtistCell: UITableViewCell{
...
class MyMusicTrackCell: UITableViewCell {
